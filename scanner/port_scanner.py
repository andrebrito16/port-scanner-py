...
from scanner.etc.helper import read_input, get_domain
...
class PortScanner:
    ...
    def __init__(self, target_ports=None, thread_limit=100, timeout=10, verbose=True, output_closed=False, start_port=0, end_port=65535):
        """
        ...
        self.__output_closed = output_closed
        ...
        if target_ports is None:
            ...
            targets_without_range_filter = self.extract_list(target_ports)
            targets_with_filter = list(filter(lambda x: self.__start_port <= x <= self.__end_port, targets_without_range_filter))
            self.targets = targets_with_filter
        ...
        if self.__verbose:
            for port in self.targets:
                if output[port] == 'OPEN':
                    ...
                    print(f'{port_proto:10}: {output[port]:>10} {service.service_name.upper():<20}\n')

        return output if self.__output_closed else ""
